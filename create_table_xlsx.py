...
workbook = xlsxwriter.Workbook('./{}'.format(FILE_NAME))
summary_format=workbook.add_format()
summary_format.set_font_color('red')
summary_format.set_align('center')
summary_date_format=workbook.add_format()
...
def chart_def(worksheet, f_short_name):
...
def summary_data_fill(summary_sheet, f_short_name, summary_idx):
    with open('{}/{}.csv'.format(RAW_DATA_FOLDER, f_short_name), 'rb') as file:
        ...
        for row in reader:
            ...
            break
    index = summary_idx + 2
    summary_sheet.write_url(index, 0, "internal:%s!A268"%(f_short_name), None, "%s"%(stock_name))
    for idx in range (10):
        summary_sheet.write(index, 1+idx,"=%s!W%s"%(f_short_name,str(249-idx))) #start at B2
...
def main():
    ...
    summary_sheet.set_column(0, 0, 11) # set column width 
...
#        print '%6s'%(f_short_name) + " creating table...",
        ...
        worksheet = workbook.add_worksheet(f_short_name)
        worksheet.write_row('A1', titles, format)
        worksheet.set_column(0, 0, 15) # A1: set column width to 15 
        worksheet.set_column(6, 6, 11) # G1: set column width to 15 
        worksheet.set_column(13, 14, 11) # N1,O1: set column width to 15 
        worksheet.set_column(16, 17, 15) # Q1,R1: set column width to 15 
        worksheet.set_column(19, 20, 13) # T1,U1: set column width to 15 
        worksheet.set_column(22, 22, 17) # W1: set column width to 15 

        # formula
        formula(worksheet)

        # chart
        chart_def(worksheet, f_short_name)

        # merge data
        merge_data(worksheet, f_name)

        # fill data at summary sheet
        if (summary_date_create == False):
            for idx in range (0,10,1):
                summary_sheet.write(1, 1+idx,"=%s!A%s"%(f_short_name,str(249-idx)), format) #start at B2
