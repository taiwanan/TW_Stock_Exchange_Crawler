...
def get_last_rows_of_CSV(f_name):
    with open('{}/{}'.format(TSE_RAW_DATA_FOLDER, f_name), 'rb') as file:
...
def summary_data_fill(summary_sheet, raw_data_folder, f_short_name, summary_idx):
    with open('{}/{}.csv'.format(raw_data_folder, f_short_name), 'rb') as file:
        ...
        for row in reader:
            ...
            stock_name = u'{:<8} {}'.format(stock_no, row[0].decode('utf-8'))
            break
...
def main():
    ...
    tse_summary_sheet.conditional_format('B3:L35800', {'type':     'cell',
    ...
    otc_summary_sheet.conditional_format('B3:L35800', {'type':     'cell',
                                        'criteria': '<=', 'value':    0.6, 'format': summary_format})
    ...
    for stock_type, raw_data, summary_sheet, total_files in zip(stock_type_list, RAW_DATA_FOLDER, summary_sheet_list, total_files_list):
        ...
        for filename in glob.glob("./{}/*.csv".format(raw_data)):
            ...
            process(stock_type, f_short_name, total_files, summary_idx)

        print ''
        # execute OTC task, reset index
        summary_idx = 0
